...
import crafter.globalvar as save_yaml

import cv2
import crafter.check_techTree as check_techTree
def main():
  ...
  parser.add_argument('--record', type=pathlib.Path, default="")
  ...
  parser.add_argument('--change_npc', type=boolean, default=True)
  ...
  parser.add_argument('--change_drink', type=boolean, default=True)
  parser.add_argument('--drink', type=list, default=['water', 'lava'])
  parser.add_argument('--change_walkable', type=boolean, default=True)
  ...
  # constants.read_world(args.record / 'world.yaml')

  # crafter.constants._world_yaml.items['health']['max'] = args.health
  # crafter.constants._world_yaml.items['health']['initial'] = args.health
  ...
  if args.gen_world:
    while True:
      ...
      # save_terrain = crafter.Env(
      #    area=args.area, view=args.area, length=args.length, seed=args.seed, args = args
      # )
