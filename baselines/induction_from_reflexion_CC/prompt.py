...
TASK_SELECTOR_SYS = '''You are a helpful assistant trying to player a new partial [counter-commonsense] 2D game, where some game mechanics are different from Minecraft. Please choose the next task from the task pool to do in the new game. My ultimate goal is to discover as many diverse things as possible, accomplish as many diverse tasks as possible while ensuring survival and become the best player in the world.
...
4) You are probably provided with possible game mechanics that have been discovered before.
...
Here's an example response (this is just an example for format, do not necessarily reflect the knowledge of the game):
Reasoning: The inventory is empty now, chop down a tree to get some wood.
Task: collect wood
'''         

## plan the specific task
TASK_PLANNER_SYS = '''You are a helper AI agent in a new partial [counter-commonsense] 2D game, where some mechanics are different from Minecraft. Based on your current inventory and observations, you need to generate the sequences of subgoals for a certain task. Please refer the history dialogue to give the plan consist of template. Do not explain or give any other instruction.
...
6) You are probably provided with some past successful plans to refer to.
7) You are probably provided with possible game mechanics that have been discovered. Please refer to the mechanism of the game to generate the plan.
...
Here are some example for output format (these are just examples for format, do not necessarily reflect the knowledge of the game):
...
TASK_REPLANNER_USER_revise1 = '''If you think the task is too hard, you can output "OVER" to give up the current task; If you think you can complete the task, you can fix the above errors and replan the task [{task}].
'''

TASK_REPLANNER_USER = '''Please fix the above errors and replan the task [{task}].'''

REFLECTION_SYS = '''You are a helpful assistant trying to player a new partial [counter-commonsense] 2D game, where some mechanics are different from Minecraft. Here are some actions that the agent fails to perform in the game. Please give the explanation of action execution failure according to the current inventory information of the agent and history dialogue.
...
7) In the new game, it is possible that some task or creature is different from Minecraft. For example, you must need some tool to mine the tree block. Thus, when you attempt to accomplish a task multiple times but failed, please try to explore more of counter common sense knowledge.
8) You are probably provided with some past successful plans and possible game mechanics that have been discovered to refer to.

Here are some examples (these are just examples for format, do not necessarily reflect the knowledge of the game):
Question:
...
CONTROLLER_SYS = '''You are a helpful assistant trying to player a new partial [counter-commonsense] 2D game, where some mechanics are different from Minecraft. Given the current observation and the goal, you need to generate the action to complete the goal. You can only perform the following actions:
...
8) You are probably provided with possible game mechanics that have been discovered before.
...
REFLECTION_MEMORY_ASSISTANT = '''Here are some possible game mechanics that have been discovered before:
{memory}
'''

CONTROLLER_REFLECTION_SYS = '''You are a helpful assistant with inductive reasoning. Given the history trajectory, including action and observation, you need to reflect on the action execution results and give the possible mechanism of the new game. The mechanism should be consistent with the game rules and the player's inventory information. 

You should only respond in the format as described below:
RESPONSE FORMAT:
Reasoning: Based on the information I listed above and history dialogue, do reasoning about the mechanism of new game.
Mechanism: The mechanism of the new game.

Here are somes examples (these are just examples for format, do not necessarily reflect the knowledge of the game):
Reasoning: The player's health decreased by 2 after shot by arrow, indicating that the arrow of skeleton is harmful to the player.
Mechanism: The arrow of skeleton can cause damage to the player.

Reasoning: The player is facing the water block and cannot enter the water block, indicating that the player cannot swim or the water block is not walkable.
Mechanism: the water block is not walkable.

Reasoning: The player has been mining the stone block for a long time but has not yet obtained the stone, indicating that the stone block cannot be mined by hand.
Mechanism: The stone block cannot be mined by hand.

Reasoning: The player has been placing the table in the front stone block for a long time but has not yet placed the table, indicating that the table cannot be placed on the stone block.
Mechanism: The table cannot be placed on the stone block.

Reasoning: The player can place the table in the front grass block, indicating that the table can be placed on the grass block.
Mechanism: The table can be placed on the grass block.
'''



CONTROLLER_REFLECTION_SYS_revise1 = '''You are a helpful assistant with inductive reasoning. Given the history trajectory, including action and observation, you need to reflect on the action execution results and give the possible mechanism of the new game. The mechanism should be consistent with the game rules and the player's inventory information. You probably provide possible game mechanics that have been discovered before. If you cannot induce useful information, you can output "null". If you induce the same mechanism as before, you can output "null". Note that the history mechanism probably incorrect, you should refine it.
